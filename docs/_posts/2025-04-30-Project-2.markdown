...
<div markdown="ol" style="margin-bottom: 10px; margin-top: 10px; overflow: hidden; color: #ffffff; background-color: #ff2222; border-color: #bce8f1; padding: 15px; border: 1px solid transparent; border-radius: 4px;"> 
    <b>Warning:</b> You are not allowed to use global variables, pointers, or pass-by references for this assignment.
</div>
...
1. Use the provided board files, `board.cpp` and `board.h`, to display the 52-tile trail on a 2-lane path (one lane for each path type, with a total of 104 tiles) (see Figure 1). 
...
1. The Final Tile is named "Pride Rock." This tile signifies the game's endpoint and should convert the Leadership traits to Pride Points to determine and congratulate the winner!
 ...
 {% include img.html src='BoardExample.png' alt='various colors in two rows on a black background. each color tile is represented by vertical-bar, space, and another vertical-bar each row is a players state and board.' caption='Example of the 2-lane Board when players choose **different path types**' %}
 

{% include img.html src='BoardExample2.png' alt='various colors in two rows on a black background. each color tile is represented by vertical-bar, space, and another vertical-bar.' caption='Example of the 2-lane Board when players choose **the same path type**' %}
...
1. **Modify the Game Board**: Increase the number of tiles or change tile colors, customize the name of the final tile, or change the player representations on the map to symbols rather than numbers. Make sure to stick to the requirements of the number of tiles and at least 2 lanes on the board.
...
{% comment %} NOT USING ANYMORE, but students can present at rectiation as detailed in the timeline  
1. **Presentation (10 points):** Present your project during recitation or record a video of your presentation (3-5 minutes).
% 
%     1. Lecture: A sign-up sheet to present during lecture will be shared a week before class presentations
%     1. Video explaining:
%     
%         1. The project idea
...
1. **Monday or Tuesday, April 28th or 29th:** Project presentation. You may present your project during your recitation for extra credit. {% comment %}Presentation deadlines?{% endcomment %}
